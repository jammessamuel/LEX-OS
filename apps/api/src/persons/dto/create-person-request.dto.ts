...
export class CreatePersonRequestDto {
  @ApiProperty({ enum: personTypes, example: 'INDIVIDUAL' })
  @IsIn(personTypes, { message: 'Informe um tipo de pessoa válido.' })
  personType!: PersonTypeCode;

  @ApiProperty({ example: 'Pessoa Fictícia de Teste', minLength: 2, maxLength: 255 })
  ...
  @ApiPropertyOptional({
    example: '***.***.***-35',
    description: 'Aceita pontuação; a resposta é mascarada.',
  })
  ...
  @ApiPropertyOptional({
    description: 'A resposta contém somente uma versão mascarada.',
    maxLength: 32,
  })
  @Transform(({ value }) => trimmed(value))
  @IsOptional()
  ...
  @ApiPropertyOptional({ format: 'email', maxLength: 320, nullable: true })
