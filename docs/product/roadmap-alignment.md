...
| 2   | Pipeline Inteligente    | 7                   | **Estrutura completa, inteligência simulada.** Contratos de fila, ciclo de vida do job, retry, reconciliação e proveniência têm forma de produção. Arquivos `text/*` são lidos do objeto privado; OCR de PDF/imagem e os demais provedores de análise continuam determinísticos. Não existe estágio de transcrição |
...
| 4   | OCR Jurídico            | 7 (parcial)         | **Leitura real de texto, OCR simulado.** Arquivos `text/*` são lidos do armazenamento até o teto registrado de 256 KiB. PDF e imagem ainda usam fixture determinística, pois não existe provedor de OCR autorizado para acervo real                                                                                |
| 5   | Extração Semântica      | 7 (determinística)  | **Conteúdo real, interpretação simulada.** Em arquivos de texto, CPF, CNPJ, número CNJ, valor e data são recortados literalmente com offsets; não há inferência. OAB e qualificadores jurídicos ainda não têm extração própria. Em PDF/imagem, a entrada continua sendo o texto simulado do OCR                    |
| 6   | Timeline Engine         | 8                   | **Concluída com inteligência simulada.** A varredura determinística encontra datas efetivamente escritas, mantém localizadores, aceita ausência de fato datado e ordena pela data do fato; a confirmação humana preserva a extração original. PDF/imagem continuam dependentes do OCR simulado                     |
...
| 10  | Assistente Interno      | 9 + 10              | **Backend ancorado concluído.** Recupera somente fontes autorizadas, recusa sem evidência e valida citações. O adaptador Anthropic existe exclusivamente para `CASE_ARCHIVE=fictional`; acervo real continua bloqueado pelos ADR-012 e ADR-016. Uma experiência conversacional completa permanece fora do escopo   |
