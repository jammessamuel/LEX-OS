...
- `CASE_ARCHIVE` declares what this installation's case archive holds — `fictional` or `real`.
  It has no default: a process that does not declare it refuses to start. The current version also
  refuses `real` at startup in every environment, because the external gates in ADR-012 and
  ADR-016 are incomplete. Keep it `fictional` locally, on staging, and on the demo. Enabling
  `real` requires a future approved delivery backed by the signed evidence package; changing a
  variable alone is deliberately insufficient.
