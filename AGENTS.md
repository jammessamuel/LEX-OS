...
Do not implement later deliveries opportunistically. Complete and verify one vertical increment at a time. The latest accepted checkpoint is **Delivery 10 — Essential web vertical slice** from `docs/architecture/implementation-plan.md`. **Delivery 11 — Verification hardening is authorized and implemented on its branch, but is not accepted until every mandatory GitHub CI job passes.** The active boundary is limited to CI, unit/integration/API/E2E security verification, dependency review, synthetic backup/restore and reconciliation rehearsal, the operational runbook, and documentation alignment. Real AI/OCR/embedding/language-model providers, full user administration, production data, deployment automation, and post-Delivery-11 product functionality remain outside the active boundary. Do not begin a new product increment before Delivery 11 is accepted.
