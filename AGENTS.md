...
Do not implement later deliveries opportunistically. Complete and verify one vertical increment at a time. The latest accepted checkpoint is **Delivery 16 — Prompt library per legal specialty**, accepted on 2026-08-28 after every mandatory CI job passed on `main`. **No Delivery 17 is authorized.** ADR-016 closes the MVP: upload is its only inbound channel, grounded retrieval remains capped at five sources until a measured evaluation authorizes more, specialty prompts remain `DRAFT` until a named lawyer signs them, and any real provider is restricted to a demonstrably fictional archive. Active work is limited to maintenance, CI, operational verification, security fixes, and documentation alignment. Real client data, inbound e-mail/WhatsApp connectors, production OCR/embedding providers, a production mail relay, SSO, billing, public signup, court integrations, and any real-archive content leaving the system require a separately authorized delivery. External legal conditions from ADR-012 cannot be represented as complete by code or internal documentation.
